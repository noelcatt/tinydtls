...
#ifdef DTLS_RLWE
static safecrypto_t *sc_sig = NULL;
static safecrypto_t *sc_kem = NULL;
static uint8_t *rlwe_pub_key;
static int rlwe_pub_key_len;

UINT32 flags[2] = {SC_FLAG_MORE, SC_FLAG_1_CSPRNG_AES_CTR_DRBG};
sc_scheme_e sig_scheme = SC_SCHEME_SIG_BLISS;
sc_scheme_e kem_scheme = SC_SCHEME_KEM_KYBER;
SINT32 sig_set = 0;
...
static int
generate_sig() {
    printf("generate_sig(): safecrypto_create\n");
    sc_sig = safecrypto_create(sig_scheme, sig_set, flags);
    if (NULL == sc_sig) {
        fprintf(stderr, "ERROR! safecrypto_create() failed\n");
        return EXIT_FAILURE;
    }
    printf("generate_sig(): safecrypto_keygen\n");
    if (SC_FUNC_SUCCESS != safecrypto_keygen(sc_sig)) {
        fprintf(stderr, "ERROR! safecrypto_keygen() failed\n");
        return EXIT_FAILURE;
    }
    printf("generate_sig(): safecrypto_public_key_encode\n");
    if (SC_FUNC_SUCCESS != safecrypto_public_key_encode(sc_sig, &rlwe_pub_key, &rlwe_pub_key_len)) {
        fprintf(stderr, "ERROR! safecrypto_public_key_encode() failed\n");
        return EXIT_FAILURE;
    }
    dtls_debug_hexdump("generate_sig(): safecrypto_public_key_encode", rlwe_pub_key, rlwe_pub_key_len);
    return EXIT_SUCCESS;
}

static int
generate_kem() {
    printf("generate_kem(): safecrypto_create\n");
    sc_kem = safecrypto_create(SC_SCHEME_KEM_KYBER, 0, flags);
    if (NULL == sc_kem) {
        fprintf(stderr, "ERROR! safecrypto_create() failed\n");
        return EXIT_FAILURE;
    }
...
static int
get_rlwe_pub_key(struct dtls_context_t *ctx,
...
static int
get_rlwe_sig(struct dtls_context_t *ctx,
        const session_t *session,
        const safecrypto_t **result) {
    *result = sc_sig;
    return EXIT_SUCCESS;
}

static int
get_rlwe_kem(struct dtls_context_t *ctx,
        const session_t *session,
        const safecrypto_t **result) {
    *result = sc_kem;
    return EXIT_SUCCESS;
...
static int
verify_rlwe_key(struct dtls_context_t *ctx,
        const session_t *session,
...
void print_version_info(void) {
    ...
    fprintf(stderr, "libsafecrypto v%08X\n",
            version);
}
#endif /* DTLS_RLWE */
...
#endif /* DTLS_ECC */
#ifdef DTLS_RLWE
    .get_rlwe_pub_key = get_rlwe_pub_key,
    .get_rlwe_sig = get_rlwe_sig,
    .get_rlwe_kem = get_rlwe_kem,
    .verify_rlwe_key = verify_rlwe_key
#endif /* DTLS_ECC */
};
